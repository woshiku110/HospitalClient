...
import com.woshiku.mydatepicker.bizs.calendars.DPCManager;
...
import com.woshiku.mydatepicker.entities.DPInfo;
...
public class MonthView extends View {
    ...
    /**
     * @desc 这个可以根据把周六日的颜色进行更改，我在这里注释掉周六日的颜色显示
     * */
    private void draw(Canvas canvas, Rect rect, DPInfo info) {
        drawBG(canvas, rect, info);
        drawOldAndNewDate(canvas, rect, info.strG);
    ...
    private void drawBGToday(Canvas canvas, Rect rect) {
        ...
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG |
                Paint.LINEAR_TEXT_FLAG);
        paint.setColor(mTManager.colorToday());
        paint.setStrokeWidth(2.0f);
        paint.setStyle(Paint.Style.STROKE);
        canvas.drawCircle(rect.centerX(), rect.centerY(), circleRadius / 2F, paint);
    }
     ...
     * 用于把今天和过去的日期隔开仅仅颜色显示
     * */
    private void drawOldAndNewDate(Canvas canvas, Rect rect, String str) {
        ...
        if(!TextUtils.isEmpty(str)){
            ...
            try {
                Date showDate = df.parse(centerYear+"-"+centerMonth+"-"+day);
                Date nowDate = new Date();
                if(showDate.before(nowDate)){
                    isBefore = true;
                    String showDateStr = df.format(showDate);
                    String nowDateStr = df.format(new Date());
                    if(showDateStr.equals(nowDateStr)){
                        isBefore = false;
                        isToday = true;
                    }
                }else{
        ...
        mPaint.setTextSize(sizeTextGregorian);
        if(isToday){
            mPaint.setColor(getContext().getResources().getColor(R.color.newDateColor));
        }else{
            if (isBefore) {
                mPaint.setColor(getContext().getResources().getColor(R.color.oldDateColor));
            } else {
                mPaint.setColor(getContext().getResources().getColor(R.color.newDateColor));
            }
        }
        ...
        if(!TextUtils.isEmpty(str)){
            int day = Integer.parseInt(str);
            if(dateSelected.contains(centerYear+"-"+centerMonth+"-"+day)){
                mPaint.setColor(mTManager.colorToday());
                canvas.drawCircle(rect.centerX(), rect.centerY(), circleRadius / 2F, mPaint);
    ...
    private boolean changeChooseDateTextColor(String chooseDateStr,String showDateStr){
        if(chooseDateStr.equals(showDateStr)){
            return true;
        }else{
            return false;
        }
    ...
    //日期是否可以被选中
    private boolean dateIsCanChoosed(String date){
        boolean canChoose = false;
        if(!TextUtils.isEmpty(date)){
            DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
            try {
                Date showDate = df.parse(date);
                Date nowDate = new Date();
                if(showDate.before(nowDate)){
                    String showDateStr = df.format(showDate);
                    String nowDateStr = df.format(new Date());
    ...
    private BGCircle createEmptyCircle(float x, float y) {
        OvalShape ovalShape = new OvalShape();
        ovalShape.resize(0, 0);
        ShapeDrawable drawable = new ShapeDrawable(ovalShape);
        //drawable.getPaint().setColor(mTManager.colorToday());
        //drawable.getPaint().setStyle(Paint.Style.STROKE);
        //drawable.getPaint().setColor(getContext().getResources().getColor(R.color.bg));
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        ...
                        if (dateSelected.contains(date)) {
                            ...
                            if(dateIsCanChoosed(date)){
                                ...
                                BGCircle circle = createEmptyCircle(
                                        region.getBounds().centerX() + indexMonth * width,
                                        region.getBounds().centerY() + indexYear * height);
                                /*BGCircle circle = createCircle(
                                        region.getBounds().centerX() + indexMonth * width,
